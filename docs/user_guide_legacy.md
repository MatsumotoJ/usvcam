...
Setup a conda enviroment for USVCAM with the following commands:
```
...
*(Optional)* If you want GPU acceleration and your PC has an NVIDIA GPU, install the additional library with the following command: 
```
(base) C:\usvcam> conda install -c conda-forge cupy
```
...
After the recording, a data directory is generated in the current working directory. The directory contains 4 files: `vid.mp4`, the video; `snd.flac`(or `snd.dat`), the sound; `sync.csv`, the video-sound synchronization; `param.h5`, other metadata.
